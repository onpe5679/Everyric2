...
기존 ``everyric2.text.latin_hangul.transliterate_latin``의 한계를 메우는 경로다.
...
2패스 정렬 리파이너(``everyric2.alignment.refine_window``)가 en 라인에서 정렬 타깃(IPA)과
화면 표시(한글/가나)를 가를 때 이 모듈을 쓴다 — ``Unit``이 표시 1글자와 그 IPA·ARPABET
음소를 함께 들고 있어서, IPA 문자열을 나중에 거꾸로 파싱하지 않고도 그 대응을 그대로
가져올 수 있다(아래 ``Unit`` 참조).
"""
...
def pronunciations(word: str) -> list[list[str]]:
    ...
    어느 발음으로 불렀는지는 곡마다 가수마다 다르다 — 사전이 못 정하는 것을 오디오는 안다.
...
def syllabify_unknown(word: str) -> list[str]:
    ...
    통째로 두면 그 낱말만 세그 하나가 되어 **그 구간에서 카라오케가 멈춘다** — 한글 정렬
    타깃 파생(``everyric2.text.align_target``)이 OOV를 철자 길이에 비례 배분하는 것도
    같은 이유다.
