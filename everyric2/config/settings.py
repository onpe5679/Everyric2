...
class AlignmentSettings(BaseSettings):
    ...
    engine: Literal["adaptive"] = Field(
        default="adaptive",
        description="Public adaptive alignment pipeline. The internal router selects the "
        "anchor model and analysis depth for each request. The measured route is "
        ...
        "_run_new_stack_alignment). Most normal songs finish on the omniASR fast path. "
        "The public engine value is always 'adaptive'; anchor names are internal details. "
        "The adaptive stack requires audio.separator_backend="
        "'bs-polarformer-fp16'이어야 한다(아래 Settings의 "
        "cross-field validator가 어긋나면 기동 시점에 바로 실패시킨다) — 그 조합만 실측됐다 "
        "(docs/research/2026-07-30-model-replacement/ust-precision-comparison.md). "
        "Legacy engines are not valid public settings.",
    )
