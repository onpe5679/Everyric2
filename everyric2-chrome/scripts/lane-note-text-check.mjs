...
try {
  ...
  // **재기 전에 영상을 멈춘다.** 네 설정을 1.5초 간격으로 재는 동안 영상이 계속 흐르면
  // 매 측정이 서로 «다른 순간»을 보게 되고, 화면에 몇 음절이 있느냐에 따라 잉크가 통째로
  // 달라진다 — 같은 곡·같은 빌드에서 off/center 비가 0.64~0.98까지 흔들린 실측 원인이
  // 이것이었다(문턱을 아무리 잡아도 흔들리는 건 문턱이 아니라 측정이었다).
  // 멈춘 뒤에도 설정 변경이 레인을 다시 그리는지는 아래에서 직접 확인한다.
  await page.evaluate(() => {
    const v = document.querySelector('video.html5-main-video') ?? document.querySelector('video');
    v?.pause();
  });
  ...
  // 멈춘 화면에서도 설정이 실제로 반영되는가 — 반영이 안 되면 아래 비교는 «같은 그림 두 장»을
  // 재는 셈이라 무의미해진다(정지 중 렌더 루프가 멈추는 회귀를 여기서 잡는다).
  check((results.center?.noteInk ?? 0) !== (results.off?.noteInk ?? 0),
    '정지 상태에서도 설정 변경이 레인에 반영됨(측정이 유효)',
    { off: results.off?.noteInk, center: results.center?.noteInk });
  ...
  //
  // off/center는 갈린다: 정상 0.757·0.825·0.834·0.897·0.989(실브라우저 5곡, **정지 측정**)
  // vs 결함 0.493~0.689(같은 5곡, 결함 재현 렌더 — 정지 화면이라 같은 조건).
  // 문턱 0.72는 그 사이에 있다 — 정상 최저(0.757)와 0.037, 결함 최고(0.689)와 0.031 여유.
  //
  // 이 수치는 **영상을 멈춘 뒤** 잰 값이다. 재생 중에 재던 처음 보정값(0.796~0.979)은
  // 같은 곡·같은 빌드가 0.636까지 떨어지는 것이 실측돼 폐기했다 — 설정마다 다른 순간을
  // 보고 있었기 때문이고, 그 흔들림은 문턱이 아니라 측정 절차의 결함이었다(위 pause 참고).
